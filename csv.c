...
{
    fprintf(output, "ethr proto;ip proto;src psysical addr;src ip addr;src port;dst psysical addr;dst ip addr;dst port;checksum;expected checksum;sequence;ack;window;flags;\n");
}
    ...
    struct tcp_packet *tcp)
{
    ...
    case IP_PROTOCOL_TCP:
        ...
        fprintf(output, "0x%04x;0x%04x;%u;%u;%u;",
            tcp->recv_checksum, tcp->calc_checksum, tcp->sequence, tcp->ack, tcp->window);
        print_tcp_flags(tcp->flags, output);
        fprintf(output, ";\n");
        break;
